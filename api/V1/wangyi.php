 ...
 * 网易新闻热榜处理器
 *
 * 数据来源: https://gw.m.163.com/nc-main/api/v1/hqc/no-repeat-hot-list
 */

class WangyiHotSearch {
    ...
    public function handle($limit = 50) {
        try {
            ...
            if (isset($data['data']) && is_array($data['data'])) {
                ...
                } elseif (isset($data['data']['hotList']) && is_array($data['data']['hotList'])) {
                    $articles = $data['data']['hotList'];
                } else {
                    $articles = $data['data'];
                }
            ...
            foreach ($articles as $v) {
                ...
                $title = '';
                foreach (array('title', 'name', 'topicName', 'hotTitle') as $k) {
                ...
                $id = '';
                foreach (array('id', 'contentId', 'docId', 'docid', 'newsId', 'tid') as $k) {
                ...
                $hot = '';
                foreach (array('hot', 'hotValue', 'heat', 'hotScore', 'playCount', 'joinCount') as $k) {
                    if (isset($v[$k]) && (is_string($v[$k]) || is_numeric($v[$k]))) {
                        $hot = $v[$k];
                        break;
                    }
                ...
                $items[] = array(
                    'index' => count($items) + 1,
                    'title' => $title,
                    'url'   => $id ? ('https://www.163.com/dy/article/' . $id . '.html') : 'https://news.163.com/',
                    'hot'   => $hot ? (is_numeric($hot) ? number_format($hot) : $hot) : '',
                    'desc'  => isset($v['digest']) ? $v['digest'] : (isset($v['description']) ? $v['description'] : ''),
                    'pic'   => isset($v['img']) ? $v['img'] : (isset($v['imgsrc']) ? $v['imgsrc'] : (isset($v['cover']) ? $v['cover'] : ''))
                );
            }
