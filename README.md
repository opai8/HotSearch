...
- **开发者版本**（当前版本）：完整功能 + 完整注释 + 分散文件，适合研究代码、二次开发
- **线上版本**：合并压缩 + 精简注释，性能更好，适合直接上线

> 两个版本功能完全一致，区别只在于代码组织方式和注释多少。
...
│   └── clear_opcache.php ← OPcache + 页面缓存清理接口（带密钥保护）
│
├── api/                  ← API 系统
...
server {
    ...
    # ⭐ 改成你的网站根目录
    root   "C:/phpstudy_pro/WWW/demo.com";
    ...
    location ~* ^/core/ {
        ...
        # clear_opcache.php 是缓存清理接口，允许访问
        location ~* /core/clear_opcache\.php$ {
            # ⭐ 改成你的 PHP-FPM 端口（phpStudy 里看）
            fastcgi_pass   127.0.0.1:9004;
            fastcgi_index  index.php;
            fastcgi_param  SCRIPT_FILENAME  $document_root$fastcgi_script_name;
...
| 功能 | 在哪改 | 开 | 关 |
|------|--------|----|----|
| **页面级静态缓存** | `config.php` | `page_cache_enable = true` | `page_cache_enable = false` |
...
- 改了 PHP 配置/代码页面没变化？点顶部「清理缓存」按钮清 OPcache + 页面缓存
- Chrome 按 `F12` → **Network** → 勾选 **Disable cache**（打开 DevTools 时生效）
- 开发时可以把 `page_cache_enable` 设为 `false`，避免每次改代码都要清缓存
...
| `sitemap.xml` | 所有 `<loc>` 标签里的域名 | 把 `https://demo.com/` 全部替换成你的实际域名 |
| `robots.txt` | 最后一行 `Sitemap:` 后面的 URL | 把 `https://demo.com/sitemap.xml` 改成你的域名 |
...
| 功能 | 怎么开 | 效果 |
|------|--------|------|
| 页面级静态缓存 | `config.php` → `page_cache_enable` 设为 `true` | 页面加载速度提升 5-10 倍 |
...
- [ ] `core/` 目录禁止直接访问（同上，`refresh.php` 和 `clear_opcache.php` 除外）
- [ ] 确认无法通过 URL 直接访问 `.htaccess` 等隐藏文件
- [ ] 修改 OPcache 清理密钥（`config.php` → `opcache_clear_key`，不要用默认值）
- [ ] 网站后台（如果有）改默认账号密码
...
- [ ] `https://你的域名/core/clear_opcache.php?key=你的密钥` → 返回 JSON 数据，`success: true`
...
| 页面级静态缓存 | 整个页面存成 HTML，下次访问直接输出，性能提升 5-10 倍 | `index.php` + `core/fetcher.php` |
...
| OPcache 一键清理 | 顶部设置按钮，清理 OPcache + 页面缓存 | `core/clear_opcache.php` |
| 页面生成耗时 | 页脚显示，带闪电图标，方便性能观察 | `index.php` + `home/footer.php` |
| DNS 预解析 | 热门平台域名提前解析，减少跳转等待 | `home/header.php` |
...
把整个渲染好的 HTML 页面存成静态文件，下次访问直接输出，跳过所有 PHP 执行和数据读取。

- **性能提升**：5-10 倍（从几十毫秒降到几毫秒）
- **缓存位置**：`cache/page_index_*.html`
- **有效期**：默认 30 分钟（`page_cache_ttl` 配置）
- **开关**：`config.php` 中的 `page_cache_enable`
- **缓存 key**：基于影响页面显示的配置（站点名、卡片高度、列数、平台数量等）生成 md5 哈希，配置变化时自动失效

> **适合场景**：热榜数据几小时才更新一次，大部分访问都可以直接返回静态页面。
> 改了配置后页面缓存会自动失效（因为哈希值变了），不需要手动清理。
...
```php
'cache_ttl' => 6 * 3600,  // 默认 6 小时（秒为单位）
```

### 第三层：OPcache（PHP 字节码缓存）

PHP 内置的字节码缓存，把 PHP 代码编译后的结果缓存起来，避免每次都重新编译。

- **性能提升**：2-5 倍
- **开关**：php.ini 里配置（代码控制不了）
- **一键清理**：顶部导航栏的「设置」按钮，或访问 `/core/clear_opcache.php?key=你的密钥`
- **开启方法**：见下方「服务器配置」→「OPcache 开启方法」
...
#### 方案 B：概率触发全局清理（兜底保障，默认已启用）

每次访问首页时，有 1% 的概率触发一次全量扫描，把所有超过 24 小时没更新过的缓存文件都删掉（包括平台数据缓存和页面静态缓存）。

- 位置：`function.php` 第 33 行（默认已启用）
- 作用：防止某些不再被访问的平台留下永久残留文件
- 关闭方式：在 `function.php` 中注释掉 `maybeCleanExpiredCache(1, 86400);`
- 可调参数：
...
| `show_header_setting` | 顶部导航栏：是否显示"清理缓存"按钮（点击清理 OPcache + 页面缓存） | `true` |
...
| `cache_ttl` | 平台数据缓存有效期（秒） | `21600`（6 小时） |
| `page_cache_enable` | 页面级静态缓存开关 | `true` |
| `page_cache_ttl` | 页面缓存有效期（秒） | `1800`（30 分钟） |
| `opcache_clear_key` | OPcache 清理密钥 | `'opcache_clear_2026'` |
...
> **关于 opcache_clear_key**：
> - 用于保护 `/core/clear_opcache.php` 接口，防止恶意调用
> - 建议上线前修改为自己的密钥（只改 config.php 就行，其他地方会自动读取）
> - 顶部导航栏的「清理缓存」按钮会自动使用这个密钥
...
| `seo_domain` | 网站域名（末尾不要加斜杠） | `'https://demo.com'` |
...
| 文件 | 作用 | 输出 |
|------|------|------|
| `index.php` | 数据拉取 + 卡片网格渲染 + 页面静态缓存 | HTML 主体 |
| `home/header.php` | DOCTYPE / CSS / 顶部导航 / toast.js / DNS 预解析 | 页面头部 HTML |
| `home/notice-bar.php` | 通知栏（数据来源提示 / 测试页入口） | 通知栏 HTML |
| `home/footer.php` | 回到顶部按钮 / 页脚 / JS 注入 / 缓存清理函数 | 页面底部 HTML |
| `config.php` | 全局配置（被 function.php 引用） | 无输出 |
| `function.php` | 统一 require（config + helper + fetcher）+ 概率清理 | 无输出 |
| `core/fetcher.php` | 数据抓取 + 缓存读写 + 过期清理 | 无输出 |
| `core/helper.php` | 工具函数 | 无输出 |
| `core/refresh.php` | AJAX 刷新接口 | JSON |
| `core/clear_opcache.php` | OPcache + 页面缓存清理接口 | JSON |
...
**Q: 改了配置/代码，页面没变化？**
A: 大概率是缓存问题。可以：
1. 点顶部导航栏的「清理缓存」按钮（清 OPcache + 页面静态缓存）
2. 按 `Ctrl + F5` 强制刷新（清浏览器缓存）
3. 开发阶段可以把 `config.php` 里的 `page_cache_enable` 设为 `false`
...
**Q: 缓存文件越来越多？**
A: 方案 A（写入时清理）已经确保每个平台只留最新 1 份。方案 B（概率全
局清理）默认已启用，会自动清理超过 24 小时的旧文件。如果还有堆积，
可以手动清理：直接删除 `cache/` 目录下所有 `.json` 和 `.html` 文件。

**Q: 首页加载很慢？**
A: 每个平台首次访问会触发真实 HTTP 请求，约 1-3 秒。有缓存后
首页在几百毫秒内完成。开启页面级静态缓存后，二次访问只要几毫秒。
可以通过调整 `cache_ttl` 控制数据刷新频率，调整 `page_cache_ttl`
控制页面缓存时长。

**Q: 页面生成耗时显示在哪里？**
A: 在页脚「服务运行正常」后面，带闪电图标。会自动根据时间长短切换
单位：小于 1 秒显示毫秒（ms），大于等于 1 秒显示秒。
如果显示「来自缓存」，说明这次访问命中了页面静态缓存。

**Q: OPcache 清理密钥怎么改？**
A: 打开 `config.php`，找到 `opcache_clear_key`，改成你自己的字符串就行。
顶部的「清理缓存」按钮会自动读取这个配置，不用改其他地方。
建议上线前一定改掉默认密钥。

**Q: 怎么切换到 Nginx？**
A: 参考 README 里的 Nginx 配置，复制到站点管理后台。
